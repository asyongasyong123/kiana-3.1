...
╚══════════════════════════════════════════════╝${NC}"
  ...
  if command -v bc >/dev/null 2>&1; then
    MEM_NUM=$(echo "$MEMORY" | sed 's/Gi//;s/Mi//')
    [[ "$MEMORY" == *Mi ]] && MEM_NUM=$(echo "scale=4; $MEM_NUM / 1024" | bc -l)
    COST_CPU=$(echo "scale=2; $CPU * 0.000024 * 730 * 3600 * $MIN" | bc -l)
    COST_MEM=$(echo "scale=2; $MEM_NUM * 0.0000025 * 730 * 3600 * $MIN" | bc -l)
    TCOST=$(echo "scale=2; $COST_CPU + $COST_MEM" | bc -l)
    echo "If min-instances = $MIN (always running): ~\$${TCOST}/month"
    echo "If min-instances = 0 (scale to zero): pay only for actual usage (~free tier eligible)"
  else
    echo "Install bc for exact estimate; approx: ~\$3.50/month for 2vCPU/2Gi always-on"
  fi
...
EOF
  ...
  REPO_REGION="${REGION%%-*}-${REGION#*-}"
  REPO_REGION="${REPO_REGION%%-*}-${REPO_REGION#*-}"
  # Simplify repo region: use multi-region if needed, fallback to us-central1
